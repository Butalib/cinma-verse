...
import { MonthlyRevenueService } from '../../data-access/monthly-revenue.service';
...
export class MonthlyRevenueChartComponent implements AfterViewInit, OnDestroy {
  private readonly monthlyRevenueService = inject(MonthlyRevenueService);
  ...
  private fetchDataAndRender(): void {
    const sub = this.monthlyRevenueService.getMonthlyRevenue().subscribe({
      next: (response) => {

...
export class MoviesLayoutComponent {
  ...
  readonly nowShowingMoviesKpi = computed(() => this.allMovies().filter((m) => m.status === 'ACTIVE').length);
  readonly comingSoonMoviesKpi = computed(() => this.allMovies().filter((m) => m.status === 'COMING_SOON').length);
  ...
  readonly filteredMovies = computed(() => {
    ...
    if (s) items = items.filter(m => m.title.toLowerCase().includes(s) || m.id.toLowerCase().includes(s));
    const status = this.statusFilter();
    if (status !== 'all') items = items.filter(m => m.status.toLowerCase() === status.toLowerCase());
    const genre = this.genreFilter();
    if (genre !== 'all') items = items.filter(m => m.genres.map(g => g.toLowerCase()).includes(genre.toLowerCase()));
    const age = this.ageFilter();
    if (age !== 'all') items = items.filter(m => m.ageRating === age);
    const lang = this.langFilter();
    if (lang !== 'all') items = items.filter(m => m.language.toLowerCase() === lang.toLowerCase());
  ...
  readonly showingFrom = computed(() => this.filteredMovies().length === 0 ? 0 : (this.currentPage() - 1) * this.pageSize() + 1);
  readonly showingTo = computed(() => Math.min(this.currentPage() * this.pageSize(), this.filteredMovies().length));

  constructor() {
    effect(() => {
      this.searchTerm(); this.statusFilter(); this.genreFilter();
      this.ageFilter(); this.langFilter(); this.dateFrom(); this.dateTo();
...
onPageSizeChange(p: number): void { this.pageSize.set(p); this.currentPage.set(1); }

  openAddMovie(): void { this.isAddMovieOpen.set(true); }
  closeAddMovie(): void { this.isAddMovieOpen.set(false); }
  onMovieAdded(m: MovieRow): void { this.moviesService.addMovie(m); this.currentPage.set(1); this.isAddMovieOpen.set(false); }

  viewMovie(m: MovieRow): void { this.selectedMovie.set(m); this.isViewMovieOpen.set(true); }
  closeViewMovie(): void { this.isViewMovieOpen.set(false); this.selectedMovie.set(null); }

  editMovie(m: MovieRow): void { this.selectedMovie.set(m); this.isEditMovieOpen.set(true); }
  closeEditMovie(): void { this.isEditMovieOpen.set(false); this.selectedMovie.set(null); }
  onMovieUpdated(u: MovieRow): void { this.moviesService.updateMovie(u); this.isEditMovieOpen.set(false); this.selectedMovie.set(null); }
  ...
  formatDuration(min: number): string { const h = Math.floor(min / 60); const m = min % 60; return h > 0 ? `${h}h ${m}m` : `${m}m`; }
  getStatusClass(s: string): string { return s === 'ACTIVE' ? 'badge badge-active' : s === 'COMING_SOON' ? 'badge badge-coming-soon' : 'badge badge-inactive'; }
  getStatusLabel(s: string): string { return s === 'ACTIVE' ? 'Active' : s === 'COMING_SOON' ? 'Coming Soon' : 'Inactive'; }
}